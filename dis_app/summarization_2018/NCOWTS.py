...
def compute_similarity(ifname,placefile,NOW):
	...
	for l in fp:
		...
		'''if B.__contains__(count)==True:
			...
			print('Content: ',len(check))
						##################### Finally apply cowts ################################
						ofname = keyterm + '_TWEB_PNCOWTS_' + str(window) + '.txt'
	...
	#print('Content: ',len(check))
		...
		#ofname = keyterm + '_TWEB_PNCOWTS_' + str(window) + '.txt'
		ofname = 'output.txt'
		optimize(tweet_cur_window,weight,ofname,NOW,0.4,0.6)
		#print('Summarization done: ',ofname)
		window+=1
		t1 = time.time()
		print('Time Elapsed: ',t1-t0)
...
class MyTCPHandler(socketserver.BaseRequestHandler):
	...
	def handle(self):
		...
		fname, placefile, cnt = eval(query)
		print("Decoded: ",fname, placefile, cnt)

		compute_similarity(fname, placefile, cnt)
