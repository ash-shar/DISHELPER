...
#Info_Path = '/home/krudra/twitter_code/sigir_disaster/towgs/Infomap/'
#fg = open('global.txt','w')
AUX = ['be','can','cannot','could','am','has','had','is','are','may','might','dare','do','did','have','must','need','ought','shall','should','will','would','shud','cud','don\'t','didn\'t','shouldn\'t','couldn\'t','wouldn\'t','doesn\'t','can\'t','didnt','haven\'t','hasn\'t']
basepath = "/home/du3/13CS30043/BTP/System/Gandhi_Award/summarization/"

import os

os.chdir(basepath)

def process_tweet(ifname,ofname):
			...
			if tag not in TAGREJECT:
				...
				if tag=='$':
					...
					if wl[0].startswith('#')==False:
						temp.add(s)
					All.add(s)
				elif tag=='^' and word not in cachedstopwords:			### and ASPELL.check(word)==1
					w = lmtzr.lemmatize(word)
					s = w + '_N'
					All.add(s)
					#if SUBJECTIVE.__contains__(w)==False:
						#s = w + '_N'
					if len(word)>1:
                                		temp.add(s)
				elif tag=='N' and word not in cachedstopwords:			### and ASPELL.check(word)==1 
					w = lmtzr.lemmatize(word)
					s = w + '_N'
					All.add(s)
					#if SUBJECTIVE.__contains__(w)==False:
					...
					try:
						...
						x = word
					if x not in AUX and x not in cachedstopwords:
						s = x + '_V'
						All.add(s)
						if len(word)>1:
							temp.add(s)
