...
basepath = BASE_DIR+"/dis_app/summarization/"
...
def compute_similarity(ifname,breakpoint,wordcount):

	return ifname, wordcount

	B = {}
	B[int(breakpoint)] = int(wordcount)
	...
	for l in lines:
		...
		try:
			...
			################### Update coword dictionary  ##################################
			
			'''for i in range(0,len(temp)-1,1):
				for j in range(i+1,len(temp),1):
					t = (temp[i],temp[j])
					t1 = (temp[j],temp[i])
					if coword.__contains__(t)==True:
						v = coword[t]
						v+=1
						coword[t] = v
					elif coword.__contains__(t1)==True:
						v = coword[t1]
						v+=1
						coword[t1] = v
					else:
		...
		# print(B, count)
		if B.__contains__(count)==True or count == len_lines:
			# print("In if")

			'''print('kill kill: ',wordnet_similarity('kill','kill',brown_ic,semcor_ic,genesis_ic))
			print('kill die: ',wordnet_similarity('kill','die',brown_ic,semcor_ic,genesis_ic))
			print('kill injure: ',wordnet_similarity('kill','injure',brown_ic,semcor_ic,genesis_ic))
			print('kill survey: ',wordnet_similarity('kill','survey',brown_ic,semcor_ic,genesis_ic))
			sys.exit(0)'''
			# print('Breakpoint: ',count)
			L = len(T.keys())
			weight = compute_tfidf(word,count)	# weight = compute_tfidf(word,count,PLACE)
			tweet_cur_window = {}
			...
			##################### Finally apply cowts ################################
			ofname = 'output_summary.txt' # OUTPUT
			optimize(tweet_cur_window,weight,ofname,int(wordcount),0.4,0.6)
			# print('Summarization done: ',ofname)
			window+=1
			t1 = time.time()
			# print('Time Elapsed: ',t1-t0)
			file = codecs.open("output_summary.txt",'r','utf-8')
...
def compute_tfidf(word,tweet_count):
	score = {}
	discard = ['pray','prayer','updates','pls','please','hope','hoping','breaking','news','flash','update','tweet','pm','a/c','v/','w/o','watch','photo','video','picture','screen','pics','latest','plz','rt','mt','follow','tv','pic','-mag','cc','-please','soul','hoax','a/n','utc','some','something','ist','afr','guru','image','images']
        ...
        for k,v in word.items():
                if k not in discard:
                        ...
                        if PLACE.__contains__(k)==True:
                                ...
                                score[k] = val
                        elif tf>=THR:					## ASPELL.check(k)==1 and
                                score[k] = val
                        else:
                                score[k] = 0
                else:
...
# def main():
# 	try:
# 		_, ifname, breakpoint, wordcount = sys.argv # breakpint - no. of tweets for which summary is to be generated
# 	except Exception as e:
# 		print(e)
# 		sys.exit(0)
# 	compute_similarity(ifname,breakpoint,wordcount)
# 	# print('Koustav Done')
